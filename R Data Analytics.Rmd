...
2 of 9 · Getting Familiar With The Data
Learn
It's easy to lose context when we're just talking about data analysis in general. The first thing, amongst many other things we should do before carrying out any analysis is to get acquainted with our dataset. We can achieve this by answering the following questions: How much data is there? What kind of data do we actually have on hand? Is there anything "weird" that might interfere with any analyses we might need to do? Is there missing data? Answering these questions using the concepts we have learned would save us time and effort later.

But, if we don't check the data beforehand, it's easy to make some false assumptions about the data that can hinder our progress later. Maybe we think that one column looks like a number type of column, whereas it's actually been read in as a string type. Perhaps some records within the dataset were misspelled due to entry errors. There may also be inconsistencies with data formats within a column.
...
# output: [1] "This branch runs because the conditional operator was true"
```
...
3. What are the data types of each of the columns? Sometimes you may find that data will look one way, but is actually disguised as another. As mentioned above, a common example of this is numbers that are actually strings in the data.

4. What are the unique values that are present in each of the columns?
...
It's important to note specifically what the str_detect() function is doing. From a practical standpoint, it may seem that the function is simply searching for the exact word "great", but there's a subtle aspect to it. In reality, it's checking whether the specified substring "great" is present within the review string. This subtlety can become a problem when smaller words are part of longer words. Let's say that we want to look for the word "pro" in the review. This is what str_detect() function will return:
```{r}
...
PRACTICE
The recent_grads dataset is loaded in.
We want to do an analysis on all of the majors that contain the word "ARTS". Create a new column called is_arts in the recent_grads dataset that is TRUE when Major contains the word "ARTS" in it or FALSE if it doesn't.
```{r}
library(tidyverse)
...
9 of 10 · String Replacement
Learn
String replacement is similar to string detection, but it takes it one step further. In addition to detecting a particular search pattern, string replacement takes that pattern and replaces it with another one using the str_replace() function.

Let's say that we're dealing with more reviews and we're still trying to check the quality of the reviews whether they are good or not. We then come across the following review:
```{r}
review2 <- "I really enjy codnig in R and wnt to lrn more."
```
Suppose we're looking for the word "enjoy" to determine if a review is good or not. If we were to use this review unaltered, we would mistakenly miss a good review. Before analyzing this review, it is important to replace all of the misspelled words. This is where the str_replace() function comes in like so:
```{r}
library(tidyverse)
review2 <- "I really enjy codnig in R and wnt to lrn more."
str_replace(review2, pattern = "enjy", replacement = "enjoy")
# output: "I really enjoy codnig in R and wnt to lrn more."
```
...
# output: "I really enjy codnig in R."
```
If the str_replace() function doesn't find a match in the string, then no replacement will happen and the string will be returned to its original form. One limitation of the str_replace() function is that it will only replace the first instance of a pattern. We can see this in the following:
```{r}
review3 <- "I want to lrn R, and I definitely wnt to lrn more."
str_replace(review3, pattern = "lrn", replacement = "learn")
# output: "I want to learn R, and I definitely wnt to lrn more."
```
Notice that the second misspelled lrn has not been fixed. In cases like this, you should use the str_replace_all() function. The syntax and keywords are similar to str_replace(), but the key difference is that str_replace_all() will replace all pattern matches instead of just the first one it sees.
```{r}
review3 <- "I want to lrn R, and I definitely wnt to lrn more."
str_replace_all(review3, pattern = "lrn", replacement = "learn")
# output: "I want to learn R, and I definitely wnt to learn more."
```
PRACTICE
The recent_grads dataset is loaded in.

We want to shorten each instance of the word "Engineering" in the Major_category column with the abbreviation "Engr.". Modify the recent_grads dataset so that the Major_category column incorporates this change.
```{r}
library(tidyverse)
...
Learn
In the last course, we emphasized the importance of vectorization in data analysis. Vectorization allows us to quickly create new columns in our data tables(tibbles) from existing columns in our data, a process that can be achieved using the mutate() function. Throughout your programming journey, we have introduced you to functions that help vectorize operations, such as the if_else() function, which enables the vectorization of the if statement on a specified column.
In this lesson, we'll learn how to vectorize a function. Like a tradition, functions take inputs, perform a calculation on them and then return an output. Currently, we have no way of taking each of the values in a tibble column and putting them through a function to get a column of outputs. Unfortunately, this process is more complicated than passing in a single column into a function and getting back an output column. This operation is extremely common in data analysis, but it requires more explanation and depth compared to what we covered so far. This lesson will combine the concepts of vectorization and custom functions, and there's a lot of ground to cover. 


The Map Function in R
2 of 11 · Introduction To The Data
Learn
To really see the power of vectorization, we'll be analyzing a dataset that is ideally suited for this purpose. Our dataset is a collection of test scores of four different students that took five different tests for three subjects: writing, math and science. Over the course of this lesson, we'll perform different analytical tasks that will require us to vectorize different functions.

In this scenario, we'll act as the teacher for these students who is responsible for evaluating these students based on their scores. Our objective is to compute various summaries and metrics for each student. Let's have a quick look at the data:
```{r}
...
In addition to the writing, math and science scores, we also have an extra_credit column that describes how much extra credit each student received for a corresponding math assignment. We'll need to take this column into account when calculating everyone's final averages. Notice too that all the scores are character vectors. We'll need to handle this in the data cleaning as well.

PRACTICE
The dataset is contained in the scores.csv file. Load it into a variable called student_scores so that we can move forward and analyze it.
```{r}
library(tidyverse)
...
The Map Function in R
3 of 11 · Preparing Our Function
Learn
The first problem we need to address with the dataset is the presentation of each students' scores as strings. Preferably, we would want them to be numbers, so as to enable us perform calculations with them. This gives us a perfect opportunity to create a function, which we can then apply in a vectorized manner to convert the entire column of strings into numbers. Once we have this function, we can efficiently format all the score columns.

Let's revisit the data preview from the last screen to keep the data format in mind:
```{r}
head(student_scores, 3)
...
4 of 11 · The map Function
Learn
Now that we have a function to vectorize, let's learn how to apply it to a column of strings. We will use the format_score() function on each of the column values to obtain another column with individual outputs for them. This process is known as vectorizing a function.
In the diagram, the inputs represent a series of values that will act as the inputs to our function. We made an example function with add_one() which takes a value, adds one to it and returns this result.

The "outputs" represent the results of applying the add_one() function to the inputs. Our goal on this screen is to learn how to apply a function (e.g., add_one() or format_score()) to a series of values.

To achieve this, we can use the map() function from the purrr package, which is part of the tidyverse. The map() function vectorizes functions, as shown in the diagram above. Before using map(), let's discuss its input and output and how they relate to tibbles.

The map() function takes two inputs: 1) a vector or list and 2) a function to apply on this vector/list. map() will take each element of the input vector or list, apply the function to each element, and will return a list. If we code the diagram above, we get:
```{r}
library(tidyverse)
# Input vector
inputs <- 1:5
...
# [1] 6
```
If you observe the data type of the outputs, we can confirm that it's a list.
```{r}
typeof(outputs)
# output: [1] "list"
```
...
Using the given vector input_scores, map the format_score() function on all of the values in this vector. Assign the output to the variable output_scores.
After creating your output_scores variable, take third element of output_scores and assign it to the variable third_element.
```{r}
library(tidyverse)
...
5 of 11 · Creating New Columns With map()
Learn
On the last screen, we learned how to use the map() function to vectorize the format_score() function, allowing us to clean up our student_scores dataset. We saw that the map() outputs a list, but it's not entirely clear how this will allow us to actually convert the student_scores into the format we need.

The answer is pretty straightforward for us to implement as programmers. We can actually use the mutate() function to directly create the new columns with the map(). What matters is that map() creates a list that has the same number of elements as the number of rows in the data. This output list will be used as the new column in the data!
...
```
The above code uses the format_score() function to clean the writing_score column, and then creates a new column new_writing_score. We can check the columns in scores to confirm that our new column is a part of the data:
```{r}
...
# [1] 64
```
Remember that the map() function returns a list, so this is what we see in the new column. Ideally, we would only want to see a vector of values instead of a list. While there are more elegant solutions to convert the map() output into a vector, right now we can fix our problem with the unlist() function. As its name suggests, the unlist() function will convert a list into a vector, which is exactly what we need.
```{r}
student_scores <- student_scores %>%
  mutate(
...
```
As we discussed before, the map() takes each value in the writing_score column and passes it into the format_score() function. Thereafter, it takes the result of the map() and assigns it to new_writing_score.

With this in mind, you can now clean the other two score columns in the scores dataset.
...
```
6 of 11 · Two Input Variation of map
Learn
On the last screen, we cleaned up the score columns in the dataset by successfully converting them into numerical format. However, there's a limitation to the map() function: it handles only one input. How do we overcome this?

Fortunately, the purrr package contains a whole family of map() functions for different situations. The map() function is designed to work with only one input, and its cousin the map2() function is designed to handle two inputs. 
...
7 of 11 · Two Input Variation of map(contd.)
Learn
On the last screen, we learned about the map2() function, a relative of the map() function that uses two inputs instead of one. Our target is to adjust a student's math score based on how much extra credit they've earned, and we've developed a function to do this job.
...
example_tibble <- example_tible %>%
  mutate(
    sum_column = unlist(map2(x, y, add_two_values))
  )
```

This code is similar to our earlier map2() example, except that we are using tibble columns instead of vectors now. Like map(), map2() also creates a list, so we need to use the unlist() function to make sure that the new column is a vector. Note that you must be careful about the order that you put the columns inside map2(). The first column will be used as the first input in the function, and the second column will be used for the second input. Getting the order wrong will lead to unexpected errors.
...
Create a new column in the student_scores dataset called adjusted_math_score. This column should represent the math score that a student received on a test, accounting for the extra credit they got on the test.
